  ...
  return <>
      ...
      <div className={cn("fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-[#1A1F2C] text-white shadow-lg transition-transform duration-300 md:sticky md:top-0", isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0 md:w-16")}>
        <div className="flex h-16 items-center justify-between px-4 py-4 border-b border-white/10 bg-[#072536]">
          ...
          <Button variant="ghost" size="icon" onClick={toggleSidebar} className="hidden md:flex text-white hover:bg-white/10">
        ...
        <ScrollArea className="flex-1 py-4 bg-[#072536]">
          <nav className="grid gap-1 px-2">
            {sidebarNavigation.map(group => <div key={group.title} className="mb-3">
                {isSidebarOpen && <h4 className="mb-1 px-2 text-xs font-semibold text-white/60">
                    {group.title}
                  </h4>}
              ...
              if (item.children) {
                ...
                return <SidebarCollapsibleSection key={item.title} item={item} isOpen={!!openSections[item.title]} onToggle={() => toggleSection(item.title)} isSidebarOpen={isSidebarOpen} onNavClick={closeSidebar} />;
              }
              return <NavLink key={item.title} to={item.href || "#"} onClick={closeSidebar} className={({
                isActive
              }) => cn("flex items-center gap-3 rounded-lg px-3 py-2 text-white hover:bg-[#f99b23] hover:text-[#1A1F2C] transition-colors", isActive && "bg-white/10 font-medium")}>
